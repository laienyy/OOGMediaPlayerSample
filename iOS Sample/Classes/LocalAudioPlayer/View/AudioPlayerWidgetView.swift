...
class AudioPlayerWidgetView: UIView {
    ...
    /// 内容外边距
    public var contentMarginInsets: UIEdgeInsets = UIEdgeInsets(top: 12, left: 20, bottom: 12, right: 20) {
    ...
    // 播放器开关
    public var enableSwitchValueChanged: ((_ view: AudioPlayerWidgetView, _ sender: UISwitch) -> Void)?
    // 音量变化
    public var volumnSliderValueChanged: ((_ view: AudioPlayerWidgetView, _ sender: OOGSlider) -> Void)?
    // 音频名称被点击
...
extension AudioPlayerWidgetView {
    ...
    func loadSubviews() {
        ...
        soundMinimumButton.setImage(UIImage(systemName: "speaker.slash.fill"), for: .normal)
        soundMaximumButton.tintColor = #colorLiteral(red: 0.5406724811, green: 0.5406724811, blue: 0.5406724811, alpha: 1)
        soundMaximumButton.setImage(UIImage(systemName: "speaker.wave.2.fill"), for: .normal)
...
//            make.leading.equalTo(topTitleLabel.snp.trailing).offset(10)
        ...
        soundSlider.snp.remakeConstraints { make in
            ...
            make.trailing.equalTo(soundMaximumButton.snp.leading).offset(-12)
            make.centerY.equalTo(soundMinimumButton)
